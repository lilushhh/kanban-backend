...
async def read_project_by_id(project_id: UUID, project_repo: ProjectRepositoryDB = Depends(get_project_repo)):
    returned_project = await project_repo.get_by_id(project_id)
    if returned_project:
        return returned_project
    raise HTTPException(status_code=404, detail="cannot find project")

@router.post("/")
async def create_project(project: CreateProjectRequest, project_repo: ProjectRepositoryDB = Depends(get_project_repo)):
    new_project = Project(
        name=project.name_project,
        users=project.users_list
    )
    project_created = await project_repo.add_item(new_project)
    if project_created:
        return project_created
    raise HTTPException(status_code=400, detail="cannot create project")

@router.delete("/{project_id}")
async def delete_project(project_id: UUID, project_repo: ProjectRepositoryDB = Depends(get_project_repo)):
    project_to_delete = await project_repo.delete_item(project_id)
    if not project_to_delete:
        raise HTTPException(status_code=404, detail="cannot find project to delete")
    return {"message": "Project deleted successfully"}

@router.put("/{project_id}")
async def update_project(project_id: UUID, update: UpdateProjectRequest, project_repo: ProjectRepositoryDB = Depends(get_project_repo)):
    project_to_update = Project(
        id=project_id,
        name=update.name_project,
        users=update.users_list
    )
    updated_project = await project_repo.update_item(project_id, project_to_update)
    if updated_project:
